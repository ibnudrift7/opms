...
class PdfController extends ControllerAdmin
{
	...
	public function actionCreate($category)
	{
		...
		if(isset($_POST['Pdf']))
		{
			...
			$file_en = CUploadedFile::getInstance($model,'file_en');
			if ($file_en->name != '') {
				$model->file_en = $file_en->name;
				$checkDataPdf2 = Pdf::model()->find('file_en = :file_en', array(':file_en'=>$model->file_en));
				if ($checkDataPdf2 != null) {
					$model->addError('file','Ganti nama file anda, dan upload kembali, nama file ada yang sama');
				}
			}
			...
			if(!$model->hasErrors() AND $model->validate()){
				...
				try
				{
					...
					if ($file_en->name != '') {
						$file_en->saveAs(Yii::getPathOfAlias('webroot').'/images/pdf/'.$model->file_en);
					}
	...
	public function actionUpdate($id, $category)
	{
		...
		if(isset($_POST['Pdf']))
		{
			...
			$file_en = $model->file_en;//mengamankan nama file
			$model->attributes=$_POST['Pdf'];//setting semua nilai
			$model->file = $file;//mengembalikan nama file
			$model->file_en = $file_en;//mengembalikan nama file
